...
import com.dd.yunpicturebackend.utils.AuthContextHolder;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture> implements PictureService{
    ...
    public PictureVO uploadPicture(Object inputSource,
        ...
        // 校验权限 //仅图片本人或空间管理员可编辑图片
        Picture checkPicture = new Picture();
        checkPicture.setSpaceId(spaceId);
        checkPicture.setUserId(loginUser.getId());
        this.checkPictureAuth(checkPicture, loginUser);
        // 判断是新增还是删除
        Long pictureId = null;
    ...
    public Boolean editPicture(PictureEditRequest pictureEditRequest, User loginUser) {
        ...
        ThrowUtils.throwIf(!result, new BusinessException(ErrorCode.OPERATION_ERROR));
        // 刷新缓存
        cacheManager.deleteCache("yunpicture:getPictureVOById:" + String.valueOf(pictureEditRequest.getId()));
        return result;
    }
    ...
    public void checkPictureAuth(Picture picture, User loginUser) {
        ...
        //管理员拥有所有权限
        if (loginUser.getUserRole().equals(UserRoleEnum.ADMIN.getValue())) return;
        if (spaceId == null) {
            // 公共图库，仅本人或管理员可操作
            ...
            if (space.getSpaceType() == SpaceTypeEnum.PRIVATE.getValue()) {
                ...
                // 团队空间
                String action = (String) AuthContextHolder.get("picture");
                SpaceUser spaceUser = spaceUserService.lambdaQuery().eq(SpaceUser::getSpaceId, spaceId)
                        .eq(SpaceUser::getUserId, loginUser.getId())
                        .one();
                ThrowUtils.throwIf(spaceUser == null || action == null, new BusinessException(ErrorCode.NO_AUTH_ERROR));
                switch (action) {
                    case SpaceUserPermissionConstant.PICTURE_VIEW:
                        // 查看权限，仅团队内的成员可以使用
                        break;
                    case SpaceUserPermissionConstant.PICTURE_EDIT:
                        // 编辑权限
                        if (SpaceRoleEnum.EDITOR.getValue().equals(spaceUser.getSpaceRole()) || SpaceRoleEnum.ADMIN.getValue().equals(spaceUser.getSpaceRole())) {
                            break;
                        }else {
                            throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
                        }
                    case SpaceUserPermissionConstant.PICTURE_UPLOAD:
                        // 上传权限
                        if (SpaceRoleEnum.EDITOR.getValue().equals(spaceUser.getSpaceRole()) || SpaceRoleEnum.ADMIN.getValue().equals(spaceUser.getSpaceRole())) {
                            break;
                        }else {
                    ...
                    case SpaceUserPermissionConstant.PICTURE_DELETE:
                        // 删除权限
                        if (SpaceRoleEnum.EDITOR.getValue().equals(spaceUser.getSpaceRole()) || SpaceRoleEnum.ADMIN.getValue().equals(spaceUser.getSpaceRole())) {
                            break;
                        }else {
