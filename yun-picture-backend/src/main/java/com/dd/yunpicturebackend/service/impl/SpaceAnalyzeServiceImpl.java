...
import com.dd.yunpicturebackend.service.*;
    ...
    public void checkSpaceAnalyzeAuth(SpaceAnalyzeRequest spaceAnalyzeRequest, User loginUser) {
        ...
        if (spaceAnalyzeRequest.isQueryAll() || spaceAnalyzeRequest.isQueryPublic()) {
            ...
            if (space.getSpaceType() == 0) {
                ThrowUtils.throwIf(!space.getUserId().equals(loginUser.getId()), new BusinessException(ErrorCode.NO_AUTH_ERROR));
            }else {
                //如果是团队空间
                // 团队空间权限校验：仅团队成员的管理员可访问
                SpaceUser spaceUser = spaceUserService.lambdaQuery().eq(SpaceUser::getSpaceId, spaceId)
                        .eq(SpaceUser::getUserId, loginUser.getId())
                        .one();
                if (spaceUser.getSpaceRole().equals(SpaceRoleEnum.ADMIN.getValue())) {
                    return;
                }else {
                    ThrowUtils.throwIf(true, new BusinessException(ErrorCode.NO_AUTH_ERROR));
                }
            }
        }
    }
