...
import com.dd.yunpicturebackend.manager.auth.AuthCheckManager;
...
public class PictureController {
    ...
    public BaseResponse<PictureVO> getPictureVOById(@RequestParam("id") Long id, HttpServletRequest request) {
        AuthContextHolder.set("picture", SpaceUserPermissionConstant.PICTURE_VIEW);
        ThrowUtils.throwIf(id <= 0, new BusinessException(ErrorCode.PARAMS_ERROR));
        ...
        //仅返回审核后的
        ThrowUtils.throwIf(picture.getReviewStatus() != PictureReviewStatusEnum.PASS.getValue(), new BusinessException(ErrorCode.FORBIDDEN_ERROR, "图片还未审核"));
        if (cacheValue == null) {
            //存入缓存
        ...
        // 获取权限列表
        User loginUser = userService.getLoginUser(request);
        List<String> permissionList = authCheckManager.getPermissionList(space, loginUser);
...
//            }
    ...
    public BaseResponse<Boolean> editPicture(@RequestBody PictureEditRequest pictureEditRequest,
                                             HttpServletRequest request) {
        AuthContextHolder.set("picture", SpaceUserPermissionConstant.PICTURE_EDIT);
    ...
    public BaseResponse<PictureVO> uploadPicture(
            ...
            HttpServletRequest request) {
        AuthContextHolder.set("picture", SpaceUserPermissionConstant.PICTURE_UPLOAD);
        User loginUser = userService.getLoginUser(request);
    ...
    , HttpServletRequest request) {
        AuthContextHolder.set("picture", SpaceUserPermissionConstant.PICTURE_DELETE);
